(ns intelsat.main
  "Kotodama WASM entrypoint for the Intelsat clean-room actor (L4) — Clojure port.
  ...
  facts (`intelsat.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "intelsat")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"intelsat.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "intelsat-compat" :tier tier :entities entities} 200])
...
(defn start! [] :intelsat-compat/ready)
